...
func TestDoctorPrintsStableSeveritiesRemediesAndFailsOnErrors(t *testing.T) {
	...
	text := output.String()
	errorAt := strings.Index(text, "✗ ")
	warningAt := strings.Index(text, "! ")
	informationAt := strings.Index(text, "› ")
...
func TestDoctorWarningsDoNotFail(t *testing.T) {
	...
	if !strings.Contains(output.String(), "! Managed home conflict") {
...
func TestDoctorStylesSeverityAndRemedyPrefixesForTerminals(t *testing.T) {
	...
	for _, want := range []string{
		"\x1b[31m✗\x1b[0m parse ",
		"\x1b[1mremedy:\x1b[0m",
		"\x1b[36m›\x1b[0m Doctor",
	} {
